...
static uint32_t data_raw_press;
...
static void Set_Environmental_Values(float *data_t, float *data_p,float *data_h);
static void Set_Random_Motion_Values(uint32_t cnt);
static void Reset_Motion_Values(void);

/* USER CODE BEGIN PFP */
static int32_t hts221_platform_write(void *handle, uint8_t reg, const uint8_t *bufp, uint16_t len);
static int32_t hts221_platform_read(void *handle, uint8_t reg, uint8_t *bufp,uint16_t len);
static void hts221_tx_com(uint8_t *tx_buffer, uint16_t len);
static void hts221_platform_delay(uint32_t ms);

static int32_t lps22hh_platform_write(void *handle, uint8_t reg, const uint8_t *bufp, uint16_t len);
static int32_t lps22hh_platform_read(void *handle, uint8_t reg, uint8_t *bufp, uint16_t len);
static void lps22hh_tx_com( uint8_t *tx_buffer, uint16_t len );
static void lps22hh_platform_delay(uint32_t ms);
/* USER CODE END PFP */
...
void hts221_init()
{
  ...
  dev_ctx.write_reg = hts221_platform_write;
  dev_ctx.read_reg = hts221_platform_read;
  dev_ctx.mdelay = hts221_platform_delay;
...
void lps22hh_init()
{
  ...
  /* Initialize mems driver interface */
  dev_ctx.write_reg = lps22hh_platform_write;
  dev_ctx.read_reg = lps22hh_platform_read;
  dev_ctx.mdelay = lps22hh_platform_delay;
  dev_ctx.handle = &SENSOR_BUS;

  /* Wait sensor boot time */
  lps22hh_platform_delay(BOOT_TIME);
  /* Check device ID */
  whoamI = 0;
...
static void Set_Environmental_Values(float *data_t, float *data_p, float *data_h)
{
	    ...
	    if (status.h_da)
	    {
			  ...
			  *data_h = linear_interpolation(&lin_hum, data_raw_humidity);
	    ...
	    if (status.t_da)
	    {
	      ...
	      dev_ctx.mdelay = lps22hh_platform_delay;
	      dev_ctx.handle = &SENSOR_BUS;
	      lps22hh_read_reg(&dev_ctx, LPS22HH_STATUS, (uint8_t *)&status2, 1);
